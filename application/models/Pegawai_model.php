...
{	
   ...
   function cek_login($nip,$password,$bagian){

        $query=$this->db->query ("SELECT * From pegawai join bagian on pegawai.id_bagian= bagian.id_bagian where nip ='{$nip}' AND password ='{$password}' AND pegawai.id_bagian = '{$bagian}' ");
        return $query->row();
   }
    ...
    function get_all()
    {
        $this->db->join('bagian','pegawai.id_bagian=bagian.id_bagian');
        $this->db->order_by($this->primary_key);
        $query = $this->db->get($this->table_name);
        return $query->result();
    }
